...
public class HexagonTile : MonoBehaviour
{
    ...
    public void SetupMesh(List<sCorner> corners)
    {
        // Build vertices relative to tile position
        Vector3[] vertices = new Vector3[corners.Count];
        for (int i = 0; i < corners.Count; i++)
        {
            vertices[i] = corners[i].position - transform.position;
        ...
        // Build triangle indices depending on corner count
        int[] indices;
        if (corners.Count == 6)
        {
            indices = new int[12];
        ...
        // Calculate normal and rotation
        normal = Vector3.Cross(vertices[1] - vertices[0], vertices[1] - vertices[3]).normalized;
        rotation = Quaternion.LookRotation(normal);

        // Create mesh and assign to components
        Mesh mesh = new Mesh();
    ...
    public void SetupTile(Vector3 position, List<sCorner> corners, sTile tile)
    {
